...
const DEFAULTS = {
  "엔진오일 필터": { cycleKm: 10000, cycleMonths: 12, defaultMode: "distance" },
  "에어 필터": { cycleKm: 15000, cycleMonths: 12, defaultMode: "distance" },
  "캐빈 필터": { cycleKm: 0, cycleMonths: 6, defaultMode: "time" },
  "연료 필터": { cycleKm: 40000, cycleMonths: 24, defaultMode: "distance" },
};

const BASE_ITEMS = Object.keys(DEFAULTS).map((kind) => ({ key: kind, kind }));

function supportsFilterItem(kind, fuelType) {
  if (fuelType !== "ev") return true;
  return !/오일.*필터|연료.*필터/i.test(String(kind || ""));
}

function getVisibleBaseItems(fuelType) {
  return BASE_ITEMS.filter((item) => supportsFilterItem(item.kind, fuelType));
}
  ...
  useEffect(() => {
    try {
      ...
      const run = async () => {
        ...
        const target = Array.isArray(data) ? data.find((n) => n.type === "filter") : null;
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        ...
        const rows = Array.isArray(data) ? data.filter((r) => supportsFilterItem(r.kind || r.label, fuelType)) : [];
        setItems(
      ...
      <div className="space-y-6 px-4 py-6 pb-32">
        ...
        <section className="rounded-2xl border border-border-light bg-surface-light p-5 shadow-card">
          <div className="flex flex-col gap-2">
            <h1 className="text-xl font-bold text-text-light">필터 관리</h1>
            <p className="text-sm text-subtext-light">엔진오일·에어·캐빈 필터 교체 주기를 한눈에 확인하세요.</p>
          </div>
