      ...
      <main className="flex-1 px-6 py-6">
        <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
          <label className="flex flex-col gap-2">
            ...
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="예) user@example.com" className="h-14 rounded-lg border border-border-light bg-surface-light px-4 text-base focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40" required />
          </label>

          <label className="flex flex-col gap-2">
            <span className="text-base font-medium">비밀번호</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="문자, 숫자를 조합해 입력해주세요" className="h-14 rounded-lg border border-border-light bg-surface-light px-4 text-base focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40" required />
          </label>
          ...
          <label className="flex flex-col gap-2">
            ...
            <input
              ...
              className={`h-14 rounded-lg border px-4 text-base focus:outline-none focus:ring-2 ${confirmPassword && password !== confirmPassword ? "border-status-danger focus:border-status-danger focus:ring-status-danger/40" : "border-border-light bg-surface-light focus:border-primary focus:ring-primary/40"}`}
              required
            />
            {confirmPassword && password !== confirmPassword ? <span className="text-sm text-status-danger">비밀번호가 일치하지 않습니다.</span> : null}
          </label>
          ...
          <section className="rounded-xl border border-border-light bg-surface-light px-4 py-3">
            ...
            <div className="flex flex-col">
              <AgreementRow checked={agreements.terms} onChange={() => toggleAgreement("terms")} label="(필수) 서비스 이용약관 동의" />
              <AgreementRow checked={agreements.privacy} onChange={() => toggleAgreement("privacy")} label="(필수) 개인정보 수집·이용 동의" />
              <AgreementRow checked={agreements.marketing} onChange={() => toggleAgreement("marketing")} label="(선택) 마케팅 정보 수신 동의" />
            </div>
          </section>

          {error ? <p className="text-sm text-status-danger">{error}</p> : null}

          <button type="submit" disabled={!requiredChecked || loading} className={`flex h-14 items-center justify-center rounded-lg text-base font-bold transition ${requiredChecked && !loading ? "bg-primary text-white hover:bg-primary/90" : "bg-border-light text-subtext-light"}`}>
