  ...
  const fetchDistance = useCallback(async () => {
    ...
    try {
      ...
      if (distanceMode === "recent") {
        ...
        const prevTo = new Date(now.getTime() - 31 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
        const prevFrom = new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    <section className="rounded-2xl border border-border-light bg-surface-light p-5 shadow-card">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-subtext-light">기간별 이동 거리와 직전 구간 대비 변화를 확인하세요.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <select value={distanceMode} onChange={(e) => setDistanceMode(e.target.value)} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
            <option value="recent">최근 30일</option>
            <option value="range">기간 지정</option>
          </select>
          {distanceMode === "range" ? (
            <>
              <select value={rangeMode} onChange={(e) => setRangeMode(e.target.value)} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
                <option value="monthly">월별 조회</option>
                <option value="date">직접 입력</option>
              </select>
              {rangeMode === "monthly" ? (
                <>
                  <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
                    {Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - i).map((y) => <option key={y} value={y}>{y}년</option>)}
                  </select>
                  <select value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
                    {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => <option key={m} value={m}>{m}월</option>)}
                  </select>
                </>
              ) : (
                <>
                  <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40" />
                  <span className="text-subtext-light">~</span>
                  <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="h-10 rounded-lg border border-border-light bg-background-light px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40" />
                </>
              )}
              <button type="button" onClick={fetchDistance} className="flex h-10 items-center gap-1 rounded-lg bg-primary px-3 text-sm font-semibold text-white transition hover:bg-primary/90 disabled:opacity-60" disabled={loading}>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border-light bg-background-light p-4">
          ...
          <p className="mt-2 text-sm text-subtext-light">최근 30일은 이전 30일, 월별은 직전 월과 비교합니다.</p>
        </div>
      </div>
    </section>
  );
