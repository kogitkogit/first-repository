...
const AGREEMENT_STORAGE_KEY = "carcare_terms_agreed_v1";

const AGREEMENT_SUMMARY = [
  "차량번호, 주행거리, 정비/주유/충전/소모품 기록 등 사용자가 입력한 차량 관리 정보를 수집하고 저장합니다.",
  "수집한 정보는 차량 관리 기능 제공, 기록 저장, 비용/정비 분석, 사용자 문의 대응을 위해 사용됩니다.",
...
export default function LoginScreen({ onLoginSuccess }) {
  ...
  const handleConsentContinue = () => {
    if (!agreed) {
      showToast({ tone: "warning", message: "필수 약관에 동의해주세요.", placement: "center", duration: 1800 });
      return;
    }
      ...
      <div className="flex min-h-screen flex-col bg-background-light text-text-light">
        <main className="flex flex-1 flex-col justify-center px-6 py-8">
          <div className="mb-8 text-center">
            ...
            <h1 className="text-3xl font-bold">CarCare 시작하기</h1>
            <p className="mt-3 text-base text-subtext-light">원하는 방식으로 시작할 수 있습니다.</p>
          </div>

          <div className="space-y-3">
            <button
              type="button"
              className="flex w-full flex-col items-start rounded-2xl border border-primary bg-primary px-5 py-4 text-white shadow-card transition hover:bg-primary/90"
              onClick={() => setMode("register")}
            >
              <span className="text-base font-bold">회원가입해서 시작하기</span>
              <span className="mt-1 text-sm text-white/80">계정으로 로그인해 데이터 백업과 복구를 지원합니다.</span>
            </button>
            ...
            >
              ...
              <span className="mt-1 text-sm text-subtext-light">빠르게 시작할 수 있지만, 앱 삭제 또는 기기 변경 시 데이터 복구가 어려울 수 있습니다.</span>
            </button>

            <button
              type="button"
              className="flex w-full items-center justify-center rounded-2xl border border-border-light bg-background-light px-5 py-4 text-base font-semibold text-subtext-light transition hover:text-primary"
        ...
        <section className="rounded-3xl border border-border-light bg-surface-light p-5 shadow-card">
          ...
          <label className="mt-4 flex items-start gap-3 rounded-2xl border border-border-light bg-background-light px-4 py-4">
            ...
            <span className="text-sm leading-6 text-text-light">
              차량 관리 서비스 이용과 개인정보 수집·이용에 동의합니다. 비회원으로 시작하는 경우 데이터 복구가 어려울 수 있음을 이해했습니다.
            </span>
          </label>
        </section>
