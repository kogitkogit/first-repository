...
const LoginScreen = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!access_token) {
        showToast({ tone: "error", message: "로그인에 실패했습니다. 다시 시도해주세요." });
        return;
      }
      ...
      showToast({ tone: "error", message: "로그인에 실패했습니다. 아이디와 비밀번호를 확인해주세요." });
    }
  };
    ...
    <div className="flex min-h-screen flex-col bg-background-light text-text-light">
      <div className="flex flex-1 flex-col px-6 py-8">
        <div className="flex justify-center py-8">
          <span className="text-primary material-symbols-outlined text-5xl">directions_car</span>
        </div>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleLogin}>
          ...
          <div className="flex items-center justify-between text-sm text-subtext-light">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={rememberUsername} onChange={() => setRememberUsername((prev) => !prev)} className="h-4 w-4 rounded border-border-light text-primary focus:ring-primary" />
              <span>아이디 기억하기</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={rememberPassword} onChange={() => setRememberPassword((prev) => !prev)} className="h-4 w-4 rounded border-border-light text-primary focus:ring-primary" />
              <span>비밀번호 기억하기</span>
            </label>
          </div>

          <button type="submit" className="mt-4 flex h-14 items-center justify-center rounded-lg bg-primary text-base font-bold text-white transition hover:bg-primary/90">
            로그인
          </button>
        </form>
        ...
        <div className="mt-8 flex items-center justify-center gap-6 text-sm text-subtext-light">
          ...
          <button type="button" onClick={() => setShowRegister(true)} className="font-medium text-primary">회원가입</button>
        </div>
      </div>
    </div>
  );
