...
function CenterModal({ title, onClose, children, onSave, saveLabel = "저장" }) {
  return (
      ...
      <div className="flex min-h-screen w-screen items-center justify-center p-4">
        <div className="w-full max-w-md rounded-3xl bg-surface-light p-6 shadow-2xl">
          ...
          <div className="mt-6 grid gap-3">
            <button type="button" className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/90" onClick={onSave}>
              {saveLabel}
            </button>
            <button type="button" className="rounded-xl border border-border-light px-4 py-2 text-sm font-semibold text-subtext-light hover:text-primary" onClick={onClose}>
              취소
            </button>
          </div>
        </div>
...
function DetailModal({ title, onClose, onEdit, onDelete, rows }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/40">
      <div className="flex min-h-screen w-screen items-center justify-center p-4">
        <div className="w-full max-w-md rounded-3xl bg-surface-light p-6 shadow-2xl">
...
export default function FuelPanel({ vehicle, onCostDataChanged = () => {} }) {
  ...
  const latestFuel = useMemo(() => fuelRecords[0] || null, [fuelRecords]);
  ...
  }, [showCharge, showFuel, vehicle?.id]);
  ...
  function validateDateInput(dateValue) {
    if (!validatePastOrToday(dateValue)) {
  ...
  async function saveFuel() {
    if (!fuelForm.date || !fuelForm.liters || !fuelForm.price_total || !fuelForm.odo_km) {
      showToast({ tone: "warning", message: "모든 필수 항목을 입력해주세요.", placement: "center", duration: 1800 });
      return;
    }
    if (!validateDateInput(fuelForm.date)) return;
    const payload = {
  ...
  async function saveCharge() {
    ...
    if (!validateDateInput(chargeForm.date)) return;
    const payload = {
      vehicle_id: vehicle.id,
  ...
  async function deleteSelected() {
    ...
    setSelected(null);
    await refresh(deletedType === "charging" ? "charge" : deletedType);
    onCostDataChanged?.();
  ...
  return (
    <div className="space-y-6 pb-28">
      ...
      <section className="space-y-3">
        ...
        {!loading && visibleRecords.map((record) => tab === "charge" ? (
          <RecordCard
            ...
            title={`${Number(record.energy_kwh || 0).toFixed(1)}kWh · ${money(record.price_total)}`}
            record={record}
            chips={
              <>
                ...
                <span className="rounded-full bg-primary/10 px-2.5 py-0.5 font-semibold text-primary">{record.charge_type === "fast" ? "급속" : "완속"}</span>
                <span>{Number(record.energy_kwh || 0) > 0 ? `kWh당 ${money(Math.round(Number(record.price_total || 0) / Number(record.energy_kwh || 1)))}` : ""}</span>
              </>
            }
            onView={() => setSelected({ type: "charge", record })}
            onEdit={() => openEdit(record)}
            onDelete={() => setPendingDelete({ type: "charging", id: record.id })}
          />
        ) : (
          <RecordCard
            key={`fuel-${record.id}`}
            title={`${Number(record.liters || 0).toFixed(1)}L · ${money(record.price_total)}`}
            record={record}
            chips={
              <>
                <span className="rounded-full bg-background-light px-2.5 py-0.5 font-semibold">{Number(record.odo_km || 0).toLocaleString()}km</span>
            ...
            onView={() => setSelected({ type: "fuel", record })}
            onEdit={() => openEdit(record)}
            onDelete={() => setPendingDelete({ type: "fuel", id: record.id })}
          />
        ))}
      </section>
      ...
      {formState.open && formState.type === "charge" ? (
        <CenterModal title={formState.mode === "edit" ? "충전 기록 수정" : "충전 기록 추가"} onClose={() => setFormState({ open: false, type: "charge", mode: "create", id: null })} onSave={() => saveCharge().catch(() => showToast({ tone: "error", message: "충전 기록을 저장하지 못했습니다." }))} saveLabel="저장">
          <div className="flex flex-wrap gap-2">
          ...
          <input type="number" step="0.1" className={INPUT_CLASS} placeholder="충전량 (kWh)" value={chargeForm.energy_kwh} onChange={(e) => setChargeForm((prev) => ({ ...prev, energy_kwh: e.target.value }))} />
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm text-primary">{chargePreview ? `예상 kWh당 비용 ${money(chargePreview)}` : "충전량과 총 금액을 입력하면 kWh당 비용이 표시됩니다."}</div>
        </CenterModal>
      ) : null}
      ...
      {selected?.type === "charge" ? (
        <DetailModal
          ...
          onDelete={() => setPendingDelete({ type: "charging", id: selected.record.id })}
          rows={[
            { label: "충전 날짜", value: selected.record.date },
            { label: "충전량", value: `${Number(selected.record.energy_kwh || 0).toFixed(1)} kWh` },
            { label: "총 금액", value: money(selected.record.price_total) },
            { label: "주행거리", value: `${Number(selected.record.odo_km || 0).toLocaleString()} km` },
            { label: "충전 방식", value: selected.record.charge_type === "fast" ? "급속" : "완속" },
            { label: "배터리", value: `${selected.record.battery_before_percent ?? "-"}% → ${selected.record.battery_after_percent ?? "-"}%` },
          ]}
        />
      ) : null}
      ...
      <ConfirmDialog
        ...
        confirmLabel="삭제"
        cancelLabel="취소"
        onConfirm={() => deleteSelected().catch(() => showToast({ tone: "error", message: "기록을 삭제하지 못했습니다." }))}
