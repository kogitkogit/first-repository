...
export default function SettingsPanel({ userId, vehicle }) {
  ...
  const handleToggle = async (type, nextValue) => {
    ...
    try {
      ...
      console.error(err);
      setNotifications((prev) => ({ ...prev, [type]: !nextValue }));
      showToast({ tone: "error", message: "알림 설정 저장에 실패했습니다. 다시 시도해주세요." });
    }
  };

  if (!userId || !vehicle) {
    return <div className="p-4 text-sm text-subtext-light">알림 설정을 사용하려면 차량을 선택한 뒤 다시 시도해주세요.</div>;
  }

  if (loading) {
    return <div className="p-4 text-sm text-subtext-light">알림 설정을 불러오는 중입니다...</div>;
  }

  const toggleItems = [
    { key: "oil", label: "오일 교체 알림", description: "주요 오일 교체 시기를 놓치지 않도록 안내드립니다." },
    { key: "filter", label: "필터 교체 알림", description: "에어·캐빈 필터 등 필터류 교체 시기를 알려드립니다." },
    { key: "consumable", label: "소모품 알림", description: "와이퍼, 배터리 등 기타 소모품 교체 시기를 알림으로 받아보세요." },
  ];
