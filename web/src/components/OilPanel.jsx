...
function supportsOilItem(kind, fuelType) {
  if (fuelType !== "ev") return true;
  return !/엔진오일|미션오일|변속기|연료/i.test(String(kind || ""));
}

function getVisibleBaseItems(fuelType) {
  return BASE_ITEMS.filter((item) => supportsOilItem(item.kind, fuelType));
}
...
export default function OilPanel({ currentMileage, vehicleId, fuelType, apiClient, onBack, hideLocalBack, userId }) {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        ...
        const rows = Array.isArray(data) ? data.filter((r) => supportsOilItem(r.kind || r.label, fuelType)) : [];
        setItems(
  ...
  }, [vehicleId, userId, fuelType]); // eslint-disable-line react-hooks/exhaustive-deps
